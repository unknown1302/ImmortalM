...
global UpdateStatus := ""
global TempUpdateFile := A_ScriptDir "\update.tmp"
global UpdateBat := A_ScriptDir "\restart_update.bat"


global SettingsFile := A_ScriptDir "\settings.ini"
global VersionURL := "https://raw.githubusercontent.com/unknown1302/ImmortalMacroUpdater/main/version.txt"
global ScriptURL  := "https://raw.githubusercontent.com/unknown1302/ImmortalMacroUpdater/main/MacroUpdater.ahk"
...
MainGui.AddGroupBox(
    "x10 y355 w310 h110 cFFFFFF",
    "Launcher"
)

txtLauncher := MainGui.AddEdit(
    "x20 y380 w190 h23 ReadOnly Background303030 cFFFFFF"
)
...
MainGui.AddText(
    "x10 y480 w80 h18 c888888",
    VERSION
)

MainGui.AddText(
    "x180 y480 w140 h18 Right c888888",
    AUTHOR
)

statusText := MainGui.AddText(
    "x10 y500 w310 h18 Center cAAAAAA",
    "Macro stopped."
)
...
LaunchCabal(*)
{
    ...
    {
        statusText.Text := "[" CurrentTime() "] Launcher file not found."
        return
    }

    SplitPath launcherPath, &name, &dir

    ; =========================================================
    ; SUPPORT BAT FILES
    ; =========================================================

    if (InStr(launcherPath, ".bat"))
    {
        Run(
            A_ComSpec ' /c ""' launcherPath '""',
            dir
        )
    }
    else
    ...
    statusText.Text := "[" CurrentTime() "] Launching Cabal client..."
}
...
CheckForUpdates()
{
    ...
    try
    {
        latestVersion := Trim(DownloadText(VersionURL))
...
CompareVersions(v1, v2)
{
    ...
    {
        p1 := (A_Index <= v1Parts.Length)
            ? Integer(v1Parts[A_Index])
            : 0

        p2 := (A_Index <= v2Parts.Length)
            ? Integer(v2Parts[A_Index])
            : 0
...
StartUpdate(version)
{
    global UpdateGui, UpdateProgress, UpdateStatus

    UpdateGui.Destroy()

    ; Create download UI
    UpdateGui := Gui("+AlwaysOnTop -Caption +Border", "Updating Macro")
    UpdateGui.AddText("x0 y0 w300 h30 Center Background2E2E2E cFFFFFF", "UPDATING CLIENT")
    UpdateGui.BackColor := 0x1E1E1E
    UpdateGui.SetFont("s9 cWhite", "Segoe UI")
    UpdateGui.SetDarkTitle()

    UpdateStatus := UpdateGui.AddText("x20 y20 w260", "Downloading update...")

    UpdateProgress := UpdateGui.AddProgress("x20 y50 w260 h20 Range0-100", 0)

    UpdateGui.Show("w300 h100")

    DownloadUpdateFile(version)
}
DownloadUpdateFile(version)
{
    global ScriptURL, TempUpdateFile

    try
    {
        whr := ComObject("WinHttp.WinHttpRequest.5.1")
        whr.Open("GET", ScriptURL, false)
        whr.Send()

        if (whr.Status != 200)
            throw Error("Download failed")

        file := FileOpen(TempUpdateFile, "w", "UTF-8-RAW")
        ...
        UpdateStatus.Text := "Downloading... 25%"
        UpdateProgress.Value := 25
        Sleep 300

        UpdateStatus.Text := "Downloading... 60%"
        ...
        UpdateStatus.Text := "Download complete"
        Sleep 600

        Sleep 500
        if !FileExist(TempUpdateFile)
        {
            MsgBox "Temp file missing:`n" TempUpdateFile
            return
        }
        CreateRestartScript()
    }
...
CreateRestartScript()
{
    global TempUpdateFile, UpdateBat

    bat := "@echo off`r`n"
    bat .= "timeout /t 2 >nul`r`n"
    bat .= 'copy /y "' TempUpdateFile '" "' A_ScriptFullPath '" >nul`r`n'
    bat .= 'powershell -Command "Start-Process """' A_ScriptFullPath '""" -Verb RunAs"`r`n'
    bat .= 'del "' TempUpdateFile '" >nul`r`n'
    bat .= 'del "%~f0"`r`n'

    try FileDelete(UpdateBat)

    FileAppend(bat, UpdateBat)

    Run(A_ComSpec ' /c "' UpdateBat '"', , "Hide")
    ExitApp()
}
ShowUpdatePopup(latestVersion)
{
    ...
    UpdateGui := Gui("+Owner" MainGui.Hwnd " +AlwaysOnTop -Caption +Border", "Update Available")
    ...
    UpdateGui.OnEvent(
        "Close",
        CloseUpdateModal.Bind(UpdateGui)
    )
    ...
    UpdateGui.Show("w300 h140 Center")
}

CloseUpdateModal(UpdateGui, *)
{
    global MainGui
